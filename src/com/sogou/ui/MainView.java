...
import com.sogou.R;
import com.sogou.adapter.CateRankListAdapter;
import com.sogou.constdata.ConstData;
import com.sogou.data.book_basic;
import com.sogou.db.BookDao;
import com.sogou.sogounovel.BookDetail;
import com.sogou.sogounovel.SearchPage;
import com.sogou.sogounovel.SogouNovelActivity;
import com.sogou.sogounovel.TcBookActivity;
import com.sogou.util.AsynImageLoaderUtil;
			...
			case 2: // 处理点击事件
				if (msg.obj != null) {
					...
					if (temp_result.getloc() != 1) {
						...
						new Thread() {
							public void run() {
								try {
									...
									book = new book_basic(book_name,author_name,null);
									book.setIs_loc(temp_result.getloc());
									book.setChapter_md5(ConstData.tc_url + temp_tc_url);
									book.setPic_path(FileUtil.new_dir + FileUtil.cheak_string(book_name) + "_" + FileUtil.cheak_string(author_name) +"/" + ConstData.cover_string);
						...
						edit.putString("webview_url", ConstData.tc_url
								+ temp_tc_url);
						edit.commit();
...
//		canvas.restore();
		...
		File file_temp = new File(file.getPath()+ "/" + ConstData.cover_string);
		if(!file_temp.exists()){
			try {
			...
			try {
				...
				System.out.println("saveBmp is here:"+file.getPath()+ "/" + ConstData.cover_string);
			} catch (Exception e) {
						e.printStackTrace();
		}
	}
