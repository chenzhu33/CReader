...
import com.sogou.R;
import com.sogou.adapter.DetailGridAdapter;
...
public class BookDetail extends Activity {
	...
	private List<chapter_basic> temp_list = new ArrayList<chapter_basic>();
						...
						new Thread() {
							public void run() {
								try {
									...
									book = new book_basic(book_name_web_view,
											author_name_web_view, null);
									book.setIs_loc(temp_result.getloc());
									book.setChapter_md5(ConstData.tc_url
											+ temp_tc_url);
...
//		requestWindowFeature(Window.FEATURE_NO_TITLE);
		...
		edit = sp.edit();
		for (int i = 0; i < ConstData.book_cate.length; i++) {
			book_cates.put(ConstData.book_cate[i], ConstData.goto_data[i]);
		}
				...
				new Thread() {
					public void run() {
						try {
							...
							book_basic book_temp = bookdao.getBook(book_name, author_name);
	...
	private void initData(SearchData temp_search) {
		...
		new Thread() {
			public void run() {
				...
				try {
					datas = XmlUtil.getXML(ConstData.rankurl,
