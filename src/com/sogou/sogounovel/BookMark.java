...
import com.sogou.data.bookmark_chlid_data;
import com.sogou.data.bookmark_group_data;
...
public class BookMark extends Activity{
	...
	List<bookmark_group_data> book_mark_group = new ArrayList<bookmark_group_data>();
	List<List<bookmark_chlid_data>> book_mark_child = new ArrayList<List<bookmark_chlid_data>>();
	@Override
...
//		System.out.println("onStop!");
	...
	public List<bookmark_group_data> group_data_init(){
		//groupdata
		Cursor cursor = db.query("book_mark", new String[]{"book_name"},
				null, null, "book_name", null, "update_time");
		List<bookmark_group_data> book_mark_group_temp = new ArrayList<bookmark_group_data>();
		
		cursor.moveToLast();
		while(cursor.getCount() > 0){
			String temp_str = cursor.getString(cursor.getColumnIndex("book_name"));
			book_mark_group_temp.add(new bookmark_group_data(temp_str,fm.getpic_frombookname(temp_str)));
	...
	public List<List<bookmark_chlid_data>> child_data_init(){
		//childdata
		Cursor cursor;
		List<List<bookmark_chlid_data>> book_mark_child_temp = new ArrayList<List<bookmark_chlid_data>>();
		for (int i = 0 ; i < book_mark_group.size() ; i++){
			List<bookmark_chlid_data> book_mark_clild_data_temp = new ArrayList<bookmark_chlid_data>();
			...
			while(cursor.getCount() > 0){
				...
				book_mark_clild_data_temp.add(new bookmark_chlid_data(cursor.getShort(cursor.getColumnIndex("id")) ,
						cursor.getString(cursor.getColumnIndex("book_name")) ,
