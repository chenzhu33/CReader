...
import com.sogou.data.book_line;
...
//	private int m_textColor_night = 0xffE6E6FA;
	...
	private Vector<book_line> m_lines = new Vector<book_line>();
	...
	public void openbook(String strFilePath) throws Exception {
		DesUtil ds = new DesUtil(ConstData.key_string);
...
//			}
	...
	protected Vector<book_line> pageDown() {
//		System.out.println("down ---begin ="+m_mbBufBegin);
		String strParagraph = "";
		Vector<book_line> lines = new Vector<book_line>();
		while (lines.size() < mLineCount && m_mbBufEnd < m_mbBufLen) {
			...
			if (strParagraph.length() == 0) {
				lines.add(new book_line(strParagraph,0,0));
			}
			while (strParagraph.length() > 0) {
				...
				lines.add(new book_line(strParagraph.substring(0, nSize),nSize,usedWidth));
